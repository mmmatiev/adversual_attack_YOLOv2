...
class MaxProbExtractor(nn.Module):
    """Извлекает максимальную вероятность для нужного класса из вывода YOLO."""
    ...
    def forward(self, YOLOoutput):
        ...
        h = YOLOoutput.size(2)
        w = YOLOoutput.size(3)
        # Приводим размерность: [batch, 5, 5+num_cls, h*w]
        output = YOLOoutput.view(batch, 5, 5 + self.num_cls, h * w)
        output = output.transpose(1, 2).contiguous()
        output = output.view(batch, 5 + self.num_cls, 5 * h * w)
        # Объектность:
        output_objectness = torch.sigmoid(output[:, 4, :])
        # Классовые вероятности:
        class_logits = output[:, 5:5 + self.num_cls, :]
        normal_confs = torch.nn.Softmax(dim=1)(class_logits)
        confs_for_class = normal_confs[:, self.cls_id, :]
        # Получаем функцию потерь из конфигурации через словарь:
        loss_target_func = self.config.loss_target
        confs_if_object = loss_target_func(output_objectness, confs_for_class)
